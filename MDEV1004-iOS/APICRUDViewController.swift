...
class APICRUDViewController: UIViewController, UITableViewDelegate, UITableViewDataSource
{
    ...
    override func viewDidLoad()
    {
        ...
        fetchGames { [weak self] games, error in
            DispatchQueue.main.async
            {
                if let games = games
                {
                    if games.isEmpty
                    {
                        ...
                        self?.displayErrorMessage("No games available.")
                    } else {
                        self?.games = games
    ...
    func fetchGames(completion: @escaping ([Games]?, Error?) -> Void)
    {
        ...
        // Configure Request
        guard let url = URL(string: "https://mdev1004-m2023-livesite-18la.onrender.com/api/") else
        {
            print("URL Error")
        ...
        URLSession.shared.dataTask(with: request) { data, _, error in
            ...
            do {
                ...
                {
                    if let gamesData = json?["data"] as? [[String: Any]]
                    {
                        let games = try JSONSerialization.data(withJSONObject: gamesData, options: [])
                        let decodedGames = try JSONDecoder().decode([Games].self, from: games)
                        completion(decodedGames, nil) // Success
                    } else {
    ...
    func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath)
        {
            if editingStyle == .delete
                {
                    let game = games[indexPath.row]
                    ShowDeleteConfirmationAlert(for: game) { confirmed in
                        if confirmed
                        {
                            self.deleteGames(at: indexPath)
                        }
                    }
                }
        }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?)
    {
        if segue.identifier == "AddEditSegue"
        {
            if let addEditVC = segue.destination as? AddEditAPICRUDViewController
            {
                ...
                addEditVC.gameUpdateCallback = { [weak self] in
                    self?.fetchGames { games, error in
                        if let games = games
                        {
                            self?.games = games
                            DispatchQueue.main.async {
    ...
    func ShowDeleteConfirmationAlert(for game: Games, completion: @escaping (Bool) -> Void)
    {
    ...
    func deleteGames(at indexPath: IndexPath)
    {
        let game = games[indexPath.row]
        ...
        guard let url = URL(string: "https://mdev1004-m2023-livesite-18la.onrender.com/api/delete/\(game._id)") else {
            print("Invalid URL")
            return
        }
